...
function insertar_profMate(){
  ...
  function getMateriaFiltro(){//Filtra las materias
    ...
    while($row = $sql_query->fetch_assoc()){
        echo "<tr><td>" . $row["NombreCarr"] . "</td><td>" . $row["NombreProfe"] ."</td><td>" . $row["NombreMat"] . "</td><td><button class='btn btn-default' onclick='eliminarProfbMat(". $row['Id_materia'] .",".$row['idprofesor'] .",". $row['id_carrera'] .")'><span style='color:red;'><i class='fas fa-trash-alt'></i></span></button></td></tr>";
    }
}
...
function getAllProfeMat(){ 
    ...
    $query = "SELECT profmat.Id_materia, mat.Nombre as NombreMat, c.id_carrera, c.Nombre as NombreCarr, p.idprofesor, p.nombre as NombreProfe FROM profesor_materia profmat INNER JOIN carrera c ON profmat.Id_carrera = c.id_carrera INNER JOIN profesores p ON profmat.Id_profesor = p.idprofesor INNER JOIN materia mat ON profmat.Id_materia = mat.id_materia WHERE c.id_carrera IN (SELECT id_carrera FROM carrera WHERE id_depto = " . $_POST['Allprof_mat'] . ") AND profmat.Estado = true" ;
